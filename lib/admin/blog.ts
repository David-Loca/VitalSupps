...
/**
 * Normalizes a locale-keyed text map, guaranteeing every current site locale
 * (see `locales` in lib/i18n) has a string entry. Any extra legacy locale keys
 * already present on the record (e.g. older `en`/`ca`/`uk`/`es` posts) are kept
 * as-is rather than dropped, so older content isn't destroyed on save.
 */
...
function normalizeBlogPost(blog: BlogPost): BlogPost {
  const safeLocale = typeof blog.locale === "string" && blog.locale ? blog.locale : defaultLocale;
  const normalizedSlug =
    typeof blog.slug === "string" ? blog.slug : normalizeLocaleTextMap(blog.slug);
...
async function writeLocalBlogsFile(jsonContent: string): Promise<boolean> {
  return writeLocalAdminJsonFile(BLOG_DATA_PATH, jsonContent);
}
...
export async function saveBlog(blog: BlogPost): Promise<void> {
  ...
  const wroteLocal = await writeLocalBlogsFile(content);
