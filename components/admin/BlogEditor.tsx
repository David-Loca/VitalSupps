...
export default function BlogEditor({ onSave, onDelete, initialBlog }: BlogEditorProps) {
  ...
  useEffect(() => {
    if (initialBlog) {
      ...
      const primary = (blogWithMeta.locale as BlogLocale) || published[0] || "fr";
  ...
  const applyToLocales = (
    ...
    const current: Record<BlogLocale, string> = { ...emptyLocaleTextMap(), ...blog[field] };
    const next = { ...current };
  ...
  const applyMetaDescription = (value: string, targets: BlogLocale[]) => {
    const current = { ...emptyLocaleTextMap(), ...blog.meta?.description };
  ...
  const applyMetaKeywords = (value: string, targets: BlogLocale[]) => {
    const current = { ...emptyLocaleTextMap(), ...blog.meta?.keywords };
  ...
  const getSlug = (locale: BlogLocale): string => {
    ...
    return String(slugRecord[locale] || "");
  };
  ...
  const getBlockContent = (block: BlogBlock, locale: string): string => {
    ...
    if (block.content && typeof block.content === "object") {
      return String(block.content[locale] || "");
    }
    return "";
  };
    ...
    setBlog({
      ...
      blocks: blog.blocks.map((block) => {
        ...
        if (typeof block.content === "string") {
          ...
          content = { ...emptyLocaleTextMap(), ...(block.content as Record<string, string>) };
        }
        for (const loc of targets) {
          content[loc] = value;
        }
  ...
  const addBlock = (type: BlogBlock["type"]) => {
    const newBlock: BlogBlock = {
      id: generateId(),
      type,
      content: emptyLocaleTextMap(),
      ...(type === "heading" && { level: 2 }),
      ...(type === "list" && {
        listItems: Object.fromEntries(
          Object.keys(emptyLocaleTextMap()).map((loc) => [loc, [""]])
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              const newAlt = typeof block.imageAlt === 'string' 
                                ...
                                : { ...emptyLocaleTextMap(), ...(block.imageAlt || {}), [blockLocale]: e.target.value };
